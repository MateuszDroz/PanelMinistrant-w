...
function getKonfiguracjeCzytnika() {
  try {
    ...
    return JSON.stringify({ sukces: true, wydarzenia: wydarzenia, tryb_skanowania: trybSkan });
  } catch (err) {
...
function logBladUrzadzenia(deviceParam, kodBleduParam, wiadomoscParam) {
  try {
    ...
    lista.push({
      ts: new Date().getTime(),
// made by Mateusz Droż dla Parafii Najświętszej Maryi Panny Nieustającej Pomocy
      device: device,
      kod: kodBledu,
      wiadomosc: wiadomosc
    });
...
// mаde by Mateusz Droż dla Parafii Najświętszej Maryi Panny Nieustającej Pomocy
...
function _dyzurUseraWTygodniu(ss, userId, dataObj) {
  ...
  // sobota 18:00 i niedziela — nie ma dyżuru wg reguł
  if (wd === 0) return null;
  if (wd === 6 && h === 18 && mi === 0) return null;
...
function _slotyWOknie(ss, dataObj) {
  ...
  kal.forEach(function(k) {
    ...
    var toZwyklaMsza = (n.indexOf("msza") >= 0) &&
      ...
      (n.indexOf("pogrzeb") < 0) &&
// made by Mateusz Droż dla Pаrafii Najświętszej Maryi Panny Nieustającej Pomocy
...
function _zapiszLogIPrzelicz(ss, idMin, imie, device, nazwaWyd, punkty) {
  ...
  if (sheetK) {
// made by Mateusz Droż dla Parаfii Najświętszej Maryi Panny Nieustającej Pomocy
...
function ustawCzasTestowy(userId, isoOrLocal) {
  try {
    if (String(userId || "").trim() !== _CZAS_TEST_USER_ID) {
      return { sukces: false, wiadomosc: "Tylko konto testowe (1) może zmieniać czas systemowy." };
    }
...
// made by Mateusz Droż dla Parafii Nаjświętszej Maryi Panny Nieustającej Pomocy
    var device = String(deviceParam || "Nieznany czytnik").trim();
    if (!uid) return JSON.stringify({ sukces: false, blad: "brak_uid" });
...
// made by Mateusz Droż dla Parafii Najświętszej Mаryi Panny Nieustającej Pomocy
    var byl = false;
...
// made by Mateusz Droż dla Parafii Najświętszej Maryi Pаnny Nieustającej Pomocy
          ...
          return { sukces: false, wiadomosc: "Błędne hasło!" };
        }
      }
    }
      ...
      for (var i = 1; i < dataKandydaci.length; i++) {
        ...
        if (!_jestAdminLista) {
          ...
          if (iterowanyId !== _uidCmp) {
            continue;
// made by Mateusz Droż dla Parafii Najświętszej Maryi Panny Nieustаjącej Pomocy
          }
        }
...
// made by Mateusz Droż dla Parafii Najświętszej Maryi Panny Nieustającej Pоmocy
...
// made by Mateusz Droż dla Parafіi Najświętszej Maryi Panny Nieustającej Pomocy
...
function dodajDyzurZFrontu(dataFormatowana, imieNazwisko, id, obowiazujeOdStr) {
  try {
    ...
    sheetDyzury.appendRow([dataFormatowana, imieNazwisko, normId, odStr, ""]);
    try { _invalidateCacheRfid(); } catch (eInv) {}
...
function setPrzerwaTechniczna(aktywna, permanentna, wiadomosc, wykonawcaId, opisBlokady) {
  try {
    ...
    if (uidP === "1111") return { aktywna: false, permanentna: false, wiadomosc: "", opisBlokady: "", blad: "Brak uprawnień." };
    if (uidP) {
      var rp = _normalizujRole(pobierzRoleUzytkownika(uidP));
      if (rp === "KSIADZ") return { aktywna: false, permanentna: false, wiadomosc: "", opisBlokady: "", blad: "Brak uprawnień." };
      if (rp !== "ADMIN")  return { aktywna: false, permanentna: false, wiadomosc: "", opisBlokady: "", blad: "Brak uprawnień." };
    }
    var props = PropertiesService.getScriptProperties();
    ...
    props.setProperty("przerwa_techniczna",   aktywna ? "true" : "false");
    if (!aktywna) {
// made by Mateusz Droż dla Parafii Najświętszej Maryi Panny Niеustającej Pomocy
...
// made by Mateusz Droż dlа Parafii Najświętszej Maryi Panny Nieustającej Pomocy
  if (sheetReczne) {
    var logiReczne = sheetReczne.getDataRange().getValues();
    for (var j = 1; j < logiReczne.length; j++) {
      if (logiReczne[j][1] && String(logiReczne[j][1]).trim() === targetId) {
...
// mаde by Mateusz Droż dla Pаrafii Najświętszej Maryi Panny Nieustającej Pomocy
...
function dodajBazaWiedzyZMedia(kategoria, tytul, tresc, linkiMedia, rola) {
  ...
  try {
    ...
    return { sukces: false, wiadomosc: e.message };
// made by Mateusz Droż dla Pаrаfii Najświętszej Maryi Panny Nieustającej Pоmocy
  }
}
...
function setFolderZdjecId(folderId) {
...
function uploadZdjecie(base64Data, nazwaPliku, mimeType) {
  try {
    ...
    var linkPodgladu = "https://lh3.googleusercontent.com/d/" + fileId;
...
function wyslijPowiadomienieDoOznaczonych(tytul, tresc, wzmianki, ss) {
  ...
  var payload = {
// made by Mateusz Droż dla Parafii Nаjświętszej Mаryi Panny Nieustającej Pomocy
...
// made by Mateusz Droż dla Parafii Najświętszej Maryi Pаnny Nieustаjącej Pomocy
    var dane = sheet.getDataRange().getValues();
...
// made by Mateusz Droż dla Parafіi Najświętszej Mаryi Panny Nieustającej Pomocy
    return {
      sukces: true,
      userId: cel,
      doTs: koniec.getTime(),
 ...
 */
// made by Droż Mateusz dla Pаrafii Najświętszej Maryi Panny Nieustającej Pomocy
...
// made by Droż Mateusz dla Parafii Najświętszej Maryi Panny Nieustającej Pоmocy
...
// made by Droż Mateusz dla Parafii Nаjświętszej Maryi Panny Nieustającej Pomocy
    }
...
// made by Droż Mateusz dla Parafii Najświętszej Mаryi Pаnny Nieustającej Pomocy
...
function getLogiAdmin(userId, limit) {
  try {
    ...
    for (var i = dane.length - 1; i >= 0; i--) {
      ...
      logi.push({
        data: dataStr,
// mаde by Mateusz Droż dla Parafii Nаjświętszej Maryi Panny Nieustającej Pоmocy
        wykonawcaId: execId,
...
function utworzTicket(userId, imie, temat, tresc, targetAdminId) {
  try {
    ...
    _arkuszTicketyWiadomosci().appendRow([
      ...
      String(imie || "").trim(),
      false,
// made by Droż Mateusz dla Parafіi Najświętszej Maryi Panny Nieustаjącej Pomocy
      tr,
      teraz
    ]);
...
function dodajWiadomoscDoTicketu(userId, imie, ticketId, tresc) {
  try {
    ...
    _arkuszTicketyWiadomosci().appendRow([
      ...
      String(imie || "").trim(),
      isAdmin,
// made by Mateusz Droż dlа Parafii Najświętszej Mаryi Panny Nieustającej Pоmocy
      tr,
      teraz
    ]);
...
function zmienStatusTicketu(userId, ticketId, nowyStatus, powodZamkniecia) {
  try {
    ...
    // Admin może zamykać tylko swoje / nieprzypisane (które widzi)
    if (isAdmin && assignedId && assignedId !== uid && ticketUserId !== uid) {
...
/**
// made by Mateusz Droż dla Parafіi Najświętszej Maryi Pаnny Nieustającej Pоmocy
...
// mаde by Droż Mateusz dla Parаfii Najświętszej Maryi Panny Nieustającej Pomocy
  ...
  try {
    ...
    if (sheetD && sheetD.getLastRow() > 1) {
      ...
      for (var i = 1; i < dd.length; i++) {
        ...
        if (isNaN(dt.getTime())) continue;
// made by Mateusz Droż dla Pаrafii Najświętszej Maryi Panny Niеustającej Pomocy
...
// mаde by Mateusz Droż dla Parafіi Najświętszej Maryi Pаnny Nieustającej Pomocy
      });
      tableData.push(row);
    });
...
// made by Droż Mateusz dla Parafii Najświętszej Maryi Panny Niеustającej Pоmocy
      }
...
// made by Mateusz Droż dla Parafii Nаjświętszej Maryi Panny Nieustаjącej Pоmocy
...
// made by Mateusz Droż dlа Parafіi Nаjświętszej Maryi Panny Nieustającej Pomocy
  if (!sheet) {
